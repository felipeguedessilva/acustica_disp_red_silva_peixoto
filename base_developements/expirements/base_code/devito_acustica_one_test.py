#==============================================================================
def acoustic_operator_one_exec(dx_ref,dt_ref,freq_ref,ptype):
#==============================================================================
    ...
    dtmax = np.around((min(hxv,hyv)*CFL)/(vmax),4)
    ...
    try: 
        ...
        mcoef = np.load("stencil_save/%s/mcoef_%s_%s_%d_%d_%f.npy"%(locsave,mshape,method,mvalue,nvalue,cur))
        ...
        if(save_stencil==1): np.save("stencil_save/%s/mcoef_%s_%s_%d_%d_%f"%(locsave,mshape,method,mvalue,nvalue,cur),mcoef)    
    ...
    print('shape: %s - method: %s - sou: %d - mvalue: %d - nvalue: %d'%(mshape,method,sou,mvalue,nvalue))
